...
Elde edilen niceliksel sonuçlar; çözünürlüğün 512²'ye yükseltilmesi ve VGG-19 algısal kaybının eklenmesi ile oluşturulan Geliştirilmiş Pix2Pix modelinin, Pix2Pix baseline (FID 151,84; SSIM 0,7772) ile CycleGAN (FID 54,58; SSIM 0,6578) modellerini beş metriğin tamamında geçtiğini ortaya koymaktadır (FID 45,16; SSIM 0,8295; PSNR 28,55 dB; LPIPS 0,1641; L1 6,77). Bu performans, klasik algı-bozulma ödünleşimini her iki yönde de aşan nadir bir durumu temsil etmektedir. Bulgular, GAN tabanlı yardımcı tasarım araçlarının kentsel peyzaj iş akışlarını dakikalardan saniyelere indirgeyebileceğini ve katılımcı planlama süreçlerini demokratikleştirebileceğini göstermektedir.
...
Quantitative results indicate that an Enhanced Pix2Pix model — obtained by increasing the resolution to 512² and incorporating a VGG-19 perceptual loss — outperforms both the Pix2Pix baseline (FID 151.84, SSIM 0.7772) and the CycleGAN model (FID 54.58, SSIM 0.6578) across all five metrics (FID 45.16, SSIM 0.8295, PSNR 28.55 dB, LPIPS 0.1641, L1 6.77). This represents a rare case in which the classical perception–distortion tradeoff is overcome in both directions. The findings demonstrate that GAN-based assistive design tools can reduce urban landscape rendering iterations from minutes to seconds, thereby democratising participatory planning processes.
